...
export class MemeJettonMinter implements Contract {
  ...
  async sendWithdraw(provider: ContractProvider, via: Sender, value: bigint, queryId: number = 0) {
    await provider.internal(via, {
      value: value,
      body: beginCell().storeUint(Op.withdraw_ton, 32).storeUint(queryId, 64).endCell(),
    })
  }

  async sendDepositLiquidityToDeDust(
    provider: ContractProvider,
    via: Sender,
    value: bigint,
    tonVaultAddress: Address,
    jettonVaultAddress: Address,
    minterContractAddress: Address,
    queryId: number = 0,
  ) {
    await provider.internal(via, {
      value: value,
  ...
  async sendDepositLiquidityToStonFi(
    ...
    pTonWalletOfRouterAddress: Address,
    jettonWalletOfRouterAddress: Address,
    minterContractAddress: Address,
    deadline: number,
    queryId: number = 0,
  ) {
  ...
  async sendNullifyAdmin(provider: ContractProvider, via: Sender, value: bigint, queryId: number = 0) {
    await provider.internal(via, {
      value: value,
      body: beginCell().storeUint(Op.nullify_admin, 32).storeUint(queryId, 64).endCell(),
    })
  }
  ...
  async sendChangeAdmin(provider: ContractProvider, via: Sender, newOwner: Address, queryId: number = 0) {
    await provider.internal(via, {
      value: toNano('0.05'),
      body: beginCell().storeUint(Op.change_admin, 32).storeUint(queryId, 64).storeAddress(newOwner).endCell(),
    })
  }
  ...
  async sendChangeContent(provider: ContractProvider, via: Sender, content: Cell, queryId: number = 0) {
    await provider.internal(via, {
      value: toNano('0.05'),
      body: beginCell().storeUint(Op.change_content, 32).storeUint(queryId, 64).storeRef(content).endCell(),
    })
  }
}

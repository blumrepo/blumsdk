...
const BUY_AMOUNT = ethers.parseEther('0.015')
...
async function main() {
  ...
  if (balance < BUY_AMOUNT) {
    console.error(
      `Error: Insufficient balance. Need ${ethers.formatEther(BUY_AMOUNT)} BNB but have ${ethers.formatEther(balance)} BNB`,
    )
    process.exit(1)
  }

  const txData = token.createBuyTx(BUY_AMOUNT, 0n)
